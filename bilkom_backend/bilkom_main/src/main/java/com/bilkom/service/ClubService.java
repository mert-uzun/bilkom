...
public class ClubService {
    ...
    public List<ClubDTO> getApprovedClubs() {
        return clubRepository.findByStatusAndIsActive(ClubRegistrationStatus.APPROVED, true).stream().map(ClubDTO::new).collect(Collectors.toList());
    }
    ...
    public List<ClubDTO> getClubsByExecutiveId(Long userId) {
        return clubExecutiveRepository.findByUserUserIdAndIsActiveTrue(userId).stream().map(executive -> new ClubDTO(executive.getClub())).collect(Collectors.toList());
    }
    ...
    public List<ClubDTO> getClubsByMemberId(Long userId) {
        return clubMemberRepository.findByMemberUserIdAndIsActiveTrue(userId).stream().map(member -> new ClubDTO(member.getClub())).collect(Collectors.toList());
    }
    ...
    public ClubDTO createClub(String clubName, String clubDescription, Long clubHeadId) {
        ...
        addClubMember(club.getClubId(), clubHeadId);
    ...
    public ClubDTO changeClubHead(Long clubId, Long newHeadId) {
        ...
        if (!isExecutive) {
            ...
            ClubExecutive executive = clubExecutiveRepository.findByUserUserIdAndClubClubIdAndIsActiveTrue(newHeadId, clubId).orElseThrow(() -> new BadRequestException("Executive not found"));
        ...
        // Make the previous club head an executive with a different position if not already
        if (previousHead != null && !previousHead.getUserId().equals(newHeadId)) {
            boolean isPreviousHeadExecutive = clubExecutiveRepository.existsByUserUserIdAndClubClubIdAndIsActiveTrue(previousHead.getUserId(), clubId);
            
            if (isPreviousHeadExecutive) {
                // Update the position of the previous head
                ClubExecutive previousHeadExecutive = clubExecutiveRepository.findByUserUserIdAndClubClubIdAndIsActiveTrue(previousHead.getUserId(), clubId).orElseThrow(() -> new BadRequestException("Executive not found"));
    ...
    public void removeClubExecutive(Long clubId, Long userId) {
        ...
        ClubExecutive executive = clubExecutiveRepository.findByUserUserIdAndClubClubId(userId, clubId).orElseThrow(() -> new BadRequestException("User is not an executive in this club"));
